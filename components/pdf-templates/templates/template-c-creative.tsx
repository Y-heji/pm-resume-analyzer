...
// Accent color: a muted blue-green that stays professional
const ACCENT = "#2563eb";

const S = {
  page: { padding: "46pt 50pt 42pt 50pt", fontFamily: FONT_FAMILY, fontSize: 9.5, lineHeight: 1.6, color: "#2d2d2d" },

  // Modern header with accent line
  header: { marginBottom: 16, borderBottom: `2pt solid ${ACCENT}`, paddingBottom: 12 },
  name: { fontSize: 22, fontFamily: `${FONT_FAMILY}-Bold`, color: "#111", marginBottom: 2 },
  role: { fontSize: 11, color: ACCENT, marginBottom: 6 },
  contact: { fontSize: 8.5, color: "#888" },

  // Top skills bar — horizontal, text-only
  skillsBar: { marginBottom: 14, paddingBottom: 12, borderBottom: "1pt solid #e8e8e8" },
  skillsBarLabel: { fontSize: 8, color: "#aaa", marginBottom: 4 },
  skillsBarText: { fontSize: 8.5, color: "#555", lineHeight: 1.45 },

  // Summary
  summaryLabel: { fontSize: 10.5, fontFamily: `${FONT_FAMILY}-Bold`, color: "#111", marginBottom: 4 },
  summaryText: { fontSize: 9.5, color: "#444", lineHeight: 1.6, marginBottom: 4 },

  // Section with accent left border
  section: { marginTop: 15 },
  sectionTitle: { fontSize: 11, fontFamily: `${FONT_FAMILY}-Bold`, color: ACCENT, marginBottom: 6, paddingBottom: 1 },

  // Entry
  entry: { marginBottom: 8, borderLeft: `1pt solid #e5e7eb`, paddingLeft: 10 },
  entryTitle: { fontSize: 10, fontFamily: `${FONT_FAMILY}-Bold`, color: "#1a1a1a", marginBottom: 1 },
  entrySub: { fontSize: 8.5, color: "#999", marginBottom: 2 },
  bullet: { fontSize: 9.5, color: "#444", lineHeight: 1.6, marginBottom: 1.5, paddingLeft: 8 },
  bulletMarker: { color: ACCENT },

  // Education
  eduEntry: { marginTop: 6 },
  eduTitle: { fontSize: 10, fontFamily: `${FONT_FAMILY}-Bold`, color: "#1a1a1a" },
  eduSub: { fontSize: 8.5, color: "#999" },
};

const A = {
  page: { padding: "46pt 50pt 42pt 50pt", fontFamily: FONT_FAMILY, fontSize: 9.5, color: "#2d2d2d" },
  ...
  divider: { borderBottom: `1.5pt solid ${ACCENT}`, marginBottom: 16 },
  section: { marginBottom: 14 },
  sectionTitle: { fontSize: 11, fontFamily: `${FONT_FAMILY}-Bold`, color: ACCENT, marginBottom: 6 },
  label: { fontSize: 8, color: "#888", marginBottom: 3 },
  text: { fontSize: 9.5, color: "#444", lineHeight: 1.6, marginBottom: 3 },
  tag: { fontSize: 8.5, color: "#444", marginBottom: 2 },
};

export default function TemplateC({ finalResume: data, deepAnalysis: d }: TemplateProps) {
  ...
  return (
    <Document title={h.name || "Resume"}>
      <Page size="A4" style={S.page} wrap>
        ...
        {/* Skills at top — ATS-safe horizontal bar */}
        {data.skills?.length > 0 ? (
          <View style={S.skillsBar}>
            <Text style={S.skillsBarLabel}>核心能力</Text>
            <Text style={S.skillsBarText}>{data.skills.join("  ·  ")}</Text>
          </View>
        ) : null}
        ...
        {/* Sections with left accent border on entries */}
          ...
          <View key={si} style={S.section}>
            ...
            {sec.entries.map((e, ei) => (
              <View key={ei} style={S.entry}>
                ...
                {e.bullets.map((b, bi) => (
                  <Text key={bi} style={S.bullet}><Text style={S.bulletMarker}>▸</Text> {b}</Text>
                ))}
              </View>
            ))}
          </View>
