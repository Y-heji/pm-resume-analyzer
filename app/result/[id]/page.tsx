          ...
          <div className="flex-1 min-w-0">
            ...
            {isLocked && (
              <p className="text-xs text-amber-600 mt-3 leading-relaxed">
                以上是免费版诊断结果。解锁专业版查看完整分析+AI深度优化+模拟面试。
              </p>
            )}
          </div>
        </div>
      ...
      {isLocked ? (
        ...
        <>
          {result.atsRisks.length > 0 && (
            <div className="mb-4">
              ...
              <div className="space-y-2">
                {result.atsRisks.slice(0, 2).map((risk, i) => (
              ...
              {result.atsRisks.length > 2 && (
                <div className="mt-2 p-3 bg-gray-50 rounded-lg border border-dashed border-gray-300 text-center">
                  <p className="text-xs text-gray-500">还有 {result.atsRisks.length - 2} 项风险 + {result.missingSkills.length} 项技能缺口</p>
                  <p className="text-xs text-gray-400 mt-0.5">解锁专业版查看完整分析报告</p>
                </div>
              )}
            </div>
          )}
          ...
          {result.missingSkills.length > 0 && (
            <div className="mb-4 p-4 bg-white rounded-xl border border-amber-100">
              ...
              <div className="flex flex-wrap gap-1.5">
                {result.missingSkills.slice(0, 5).map((ms, i) => (
          ...
          {result.atsRisks.length > 0 && (
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-sm font-semibold text-gray-700">ATS 风险详情</h3>
                <span className="text-xs text-gray-400">{result.atsRisks.length} 项</span>
              </div>
              <div className="space-y-2">
